...
from ly_ads_mcp.servers.search.client import LabelServiceCountLabeledEntity
from ly_ads_mcp.servers.search.tools import list_search_labels as list_search_labels_tool
from ly_ads_mcp.servers.search.tools.list_search_labels import ListSearchLabelsRequest
...
async def test_list_search_labels_maps_request_to_selector(monkeypatch, patch_to_thread):
    captured = {}
                            ...
                            label=FakeLabel(
                                {
                                    ...
                                    "labelName": "Priority",
                                    "color": "#FFFFFF",
                                    "description": "Priority campaigns",
                                    "labeledCampaign": 3,
                                    "labeledAdGroup": 4,
                                    "labeledAdGroupAd": 5,
                                    "labeledAdGroupCriterion": 6,
                                }
                            )
                        )
                    ],
    ...
    monkeypatch.setattr(list_search_labels_tool, "LabelServiceApi", FakeLabelServiceApi)
    result = await list_search_labels_tool.list_search_labels(
        FakeHandlers(),
        ListSearchLabelsRequest(
            base_account_id=123,
            account_id=456,
            label_ids=[789],
            count_labeled_entity=LabelServiceCountLabeledEntity.TRUE,
        ),
    )
    ...
    assert selector.count_labeled_entity == LabelServiceCountLabeledEntity.TRUE
    ...
    assert result.items[0].label_name == "Priority"
    assert result.items[0].labeled_campaign == 3
    assert result.items[0].labeled_ad_group_criterion == 6


async def test_list_search_labels_uses_fixed_page_size_across_pages(monkeypatch, patch_to_thread):
    captured = []
    ...
    first = await list_search_labels_tool.list_search_labels(
        handlers,
        ListSearchLabelsRequest(base_account_id=123, account_id=456),
    )
    continued = await list_search_labels_tool.list_search_labels(
        handlers,
...
async def test_list_search_labels_rejects_value_with_no_label(monkeypatch, patch_to_thread):
    class FakeLabelServiceApi:
        def __init__(self, client):
            pass
    ...
    with pytest.raises(ToolError, match="label entry without label data"):
        await list_search_labels_tool.list_search_labels(
            FakeHandlers(),
            ListSearchLabelsRequest(base_account_id=123, account_id=456),
        )


def test_list_search_labels_rejects_too_many_label_ids():
