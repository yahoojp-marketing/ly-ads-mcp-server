...
"""Schemas and implementation for the list_accessible_search_accounts MCP tool."""
...
_TOOL_NAME = "list_accessible_search_accounts"


class ListAccessibleSearchAccountsRequest(StrictRequestModel):
    ...
    base_account_id: int = Field(
        description=(
            "Search Ads baseAccountId (x-z-base-account-id context). "
...
class SearchAccountItem(McpOutputModel):
    base_account_id: int = Field(
        alias="baseAccountId",
        description="Base account ID used as the Search Ads API request context.",
    )
...
async def list_accessible_search_accounts(
    ...
    request: ListAccessibleSearchAccountsRequest | None = None,
    ...
    return await paginate(
        ...
        first_request_type=ListAccessibleSearchAccountsRequest,
        tool_name=_TOOL_NAME,
        schema_version=_CURSOR_SCHEMA_VERSION,
        fetch_page=lambda first, start, size: _fetch_accessible_search_accounts(
            handlers, first, start, size
        ),
    )


async def _fetch_accessible_search_accounts(
    handlers: SearchHandlers,
    request: ListAccessibleSearchAccountsRequest,
    start_index: int,
    number_results: int,
) -> PageSlice[SearchAccountItem]:
    ...
    for value in values:
        ...
        if account is None:
            raise ToolError(
                "LY Ads Search Ads API returned an ad-account entry without account data. "
        ...
        items.append(SearchAccountItem.model_validate(item))
